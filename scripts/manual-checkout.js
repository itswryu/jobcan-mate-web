 ...
 * 수동 퇴근 스크립트
 * 
 * 이 스크립트는 설정 파일에 있는 첫 번째 사용자의 퇴근 작업을 실행합니다.
...
async function main() {
  try {
    ...
    console.log(`사용자 ${user.email || user.id}의 퇴근 작업을 시작합니다...`);

    // 3. 퇴근 API 실행
    const apiUrl = 'http://localhost:3000/api/jobcan';
    console.log(`curl -X POST ${apiUrl} -H "Content-Type: application/json" -d '{"action":"checkOut"}'`);
    
    try {
      const result = execSync(`curl -X POST ${apiUrl} -H "Content-Type: application/json" -d '{"action":"checkOut"}'`, {
        encoding: 'utf-8'
      });
